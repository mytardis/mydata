...
def LogTestRunSummary():
    ...
    numFoundInCache = \
        DATAVIEW_MODELS['verifications'].GetFoundInCacheCount()
    ...
    logger.testrun("")
    logger.testrun("Files looked up: %s" % numVerificationsCompleted)
    logger.testrun("Files verified on server: %s"
                   % (numVerifiedUploads + numFoundInCache))
    logger.testrun("Cache hits: %s" % numFoundInCache)
    logger.testrun("Files not found on server: %s" % numFilesNotFoundOnServer)
