...
def UploadFileFromPosixSystem(filePath, fileSize, username, privateKeyFilePath,
    ...
    mkdirCmdAndArgs = \
        [OPENSSH.DoubleQuote(OPENSSH.ssh),
         ...
         "-l", username,
         host,
         OPENSSH.DoubleQuoteRemotePath("mkdir -p %s" % quotedRemoteDir)]
    ...
    rmCommandString = \
        ...
        % (OPENSSH.DoubleQuote(OPENSSH.ssh),
           ...
           sshControlPathOptionValuePair,
           username, host,
           OPENSSH.DoubleQuoteRemotePath(remoteRemoveChunkCommand))
    ...
    while bytesUploaded < fileSize:
        ...
        scpCommandString = \
            ...
            % (OPENSSH.DoubleQuote(OPENSSH.scp),
               ...
               chunkFilePath,
               username, host,
               OPENSSH.DoubleQuoteRemotePath(remoteChunkPath)
               .replace('`', r'\\\`'))
        ...
        catCommandString = \
            ...
            % (OPENSSH.DoubleQuote(OPENSSH.ssh), port,
               ...
               sshControlPathOptionValuePair,
               username, host,
               OPENSSH.DoubleQuoteRemotePath(remoteCatCommand))
...
def UploadSmallFileFromWindows(filePath, fileSize, username,
    ...
    rmCommandString = \
        ...
        % (OPENSSH.DoubleQuote(OPENSSH.ssh), port,
           ...
           OPENSSH.cipher,
           username, host,
           OPENSSH.DoubleQuoteRemotePath(remoteRemoveDatafileCommand))
    # logger.debug(rmCommandString)
    ...
    scpCommandString = \
        ...
        % (OPENSSH.DoubleQuote(OPENSSH.scp), port,
           ...
           username, host,
           quotedRemoteDir.replace('&', '^&').replace('`', r'\\\`'))
    logger.debug(scpCommandString)
